...
func run() error {
    ...
    // 💡 CORREZIONE CRITICA: Inizializzazione dello schema del database
    // Questo è NECESSARIO per creare le tabelle (come 'users') se non esistono.
    logger.Println("ensuring database schema is initialized")
    if err := db.CreateSchema(); err != nil {
        logger.WithError(err).Error("error initializing database schema")
